...
- Skip `npx eslint --plugin security` (it installs a package). `npm audit --omit=dev` is fine. Highest priority: identity data, payment webhooks and entitlement, stored-file serving, admin routes, email tokens, session handling around OAuth redirects, and any "does this account exist" answer (the words, the errors and the timing must be the same for every address).
